...
//		System.setIn(new java.io.FileInputStream("eval_input.txt"));
    ...
    static class UserSolution {
        static class Node{
            int uId, timestamp, like;

            public Node (int uId, int timestamp) {
                this.uId = uId;
        ...
        static Node[] nodes;
        static boolean[][] follows;
        static int total;
        public void init(int N)
        {
            nodes = new Node[100001];
            follows = new boolean[N+1][N+1];
            total =0;
        }
        ...
        {
            nodes[pID] = new Node(uID, timestamp);
            total++;
        }
        ...
        public void getFeed(int uID, int timestamp, int pIDList[])
        {
            ...
            for(int i=total; i>0; i--){
                if(!follows[uID][nodes[i].uId]) continue;
                 if(timestamp-nodes[i].timestamp>1000 && size<10){
                    pIDList[size++]=i;
                    if(size==10) return;
                }
                else if(timestamp-nodes[i].timestamp>1000 && size>=10)  return;
                else{
                    find(pIDList, i);
                    size++;
                }
        ...
        public void find(int [] pIDList, int pid){
            for(int j=0; j<10; j++){
                if(pIDList[j]==0) {
                    pIDList[j] = pid;
                    return;
                }
                else if(nodes[pIDList[j]].like<nodes[pid].like){
                    int k=9;
                    while(k!=j){
                        pIDList[k] = pIDList[k-1];
                        k--;
                    }
                    pIDList[j] = pid;
                    return;
